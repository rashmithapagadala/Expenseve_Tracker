...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  private expenses$ = new Subscription();
  ...
  /** Subscribe to this user's expenses using their UID (never email). */
  private subscribeToExpensesChange() {
    ...
    if (uid) {
      ...
      // no user yet; stop spinner to avoid hanging UI
  ...
  private parseData(snapsShots: SnapshotAction<Expense>[]): Expense[] {
    ...
    snapsShots.forEach((snapshot) => {
      const expense = snapshot.payload.exportVal() as Expense;
      if (!expense) return;
      expense.id = snapshot.key ?? undefined;
      data.push(expense);
    });
    return data;
  }
  ...
  private processExpenseData(expenses: Expense[]): void {
    ...
    const lastDate = dates.reduce((maxDate, date) => (date > maxDate ? date : maxDate), dates[0]);
  ...
  getSourceTotal(sources: string[], data: Expense[], property: keyof Expense): ChartData[] {
    ...
    for (const type of sources) {
      ...
      sourcePieData.push({ name: type, y: sourceSum, value: +sourceSum.toFixed(2) });
    }
